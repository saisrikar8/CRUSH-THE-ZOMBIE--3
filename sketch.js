...
function setup() {
  ...
  zombie = createSprite(100, height - 110);
  ...
  for (var i = 0; i < 16; i++){
    stone = new Stone(Math.random(bridge.pointA.x + 50, jointPoint.body.position.x + 50), 50);
    stones.push(stone);
  }
}
...
function draw() {
  ...
  for (var x = 0; x < stones.length; x++){
    stones[x].display();
    if (collide(stones[x].body, zombie)){
      zombie.velocityX = 0;
      Body.setStatic(stones[x].body, true);
    }
  }
